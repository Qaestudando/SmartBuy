...
interface MuffatoCommercialOffer {
  Price?: number;
  ListPrice?: number;
...
interface MuffatoSeller {
  sellerId?: string;
  sellerName?: string;
  commertialOffer?: MuffatoCommercialOffer;
}

interface MuffatoItem {
  itemId?: string;
  name?: string;
  nameComplete?: string;
  ean?: string;
  ...
  sellers?: MuffatoSeller[];
}

interface MuffatoProduct {
  productId?: string;
  productName?: string;
  brand?: string;
  link?: string;
...
export class MuffatoConnector implements SupermarketConnector {
  ...
  async searchProduct(item: ShoppingItem): Promise<PriceResult[]> {
    ...
    console.log(
      `[Muffato] Buscando produto pela API VTEX: "${searchTerm}"`
    );

    try {
      const response = await axios.get<MuffatoProduct[]>(
        this.searchUrl,
        {
          params: {
            ft: searchTerm,
      ...
      console.log(
        `[Muffato] API respondeu: ${response.status}`
      );

      console.log(
        `[Muffato] Produtos encontrados na API: ${response.data.length}`
      );
      ...
      console.log(
        `[Muffato] Produtos após filtro: ${filteredProducts.length}`
      );
  ...
  private extractProductName(item: ShoppingItem): string {
    return item.produto
      .trim()
      .replace(
        /\b\d+(?:[.,]\d+)?\s*(?:kg|g|ml|l|un|unidade|unidades)\b/gi,
        ""
      )
      .replace(/\s+/g, " ")
      .trim();
  ...
  private filterProducts(
    products: MuffatoProduct[],
    item: ShoppingItem
  ): MuffatoProduct[] {
    const requestedProduct = this.normalizeText(
  ...
  private productMatchesWeight(
    ...
    const productWeight = this.extractWeight(
      productName
    );
    ...
    if (productWeight !== undefined) {
      return (
        Math.abs(
          productWeight - requestedWeight
        ) < 0.0001
      );
    }

    for (const currentItem of items) {
      const itemName = this.normalizeText(
        currentItem.nameComplete ??
        currentItem.name ??
        ""
      );

      const itemWeight = this.extractWeight(
        itemName
      );
  ...
  private parseProducts(
    ...
    for (const product of products) {
      ...
      const seller = productItem.sellers?.find(
        (currentSeller) =>
          currentSeller.commertialOffer?.IsAvailable !== false
      );
      ...
      const precoAnterior = this.getPreviousPrice(
        offer,
        preco
      );
      ...
      const result: PriceResult = {
        ...
        ...(product.link
          ...
          : product.linkText
            ? {
                urlProduto:
                  `${this.baseUrl}/${product.linkText}/p`,
              }
            : {}),
      };
    ...
    console.log(
      `[Muffato] Resultados processados: ${results.length}`
    );
  ...
  private getPrice(
    ...
    if (
      typeof offer.FullSellingPrice === "number" &&
    ...
    if (
      typeof offer.PriceWithoutDiscount === "number" &&
